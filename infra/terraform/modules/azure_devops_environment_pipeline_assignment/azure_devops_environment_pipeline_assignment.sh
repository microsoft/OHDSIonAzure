...
# read from query
eval "$(jq -r '@sh "PAT=\(.pat) ADO_ORG_SERVICE_URL=\(.adoOrgServiceUrl) ADO_PROJECT_NAME=\(.adoProjectName) ENVIRONMENT_ID=\(.adoEnvironmentId) PIPELINE_ID=\(.adoPipelineId) AUTHORIZED=\(.authorized)"')"
...
resultJson=$(curl -X PATCH "$ADO_ORG_SERVICE_URL/$ADO_PROJECT_NAME/_apis/pipelines/pipelinepermissions/environment/$ENVIRONMENT_ID?api-version=7.1-preview.1" \
